...
# ==========================
st.set_page_config(
    page_title="Activity Recognition Tool",
    page_icon="assets/favicon.ico",
    layout="wide"
)

st.title("🎯 Activity Recognition: Feature Extraction & ML Comparison")
st.markdown("""
...
if uploaded_file is not None and st.session_state.df is None:
    ...
    if not all(col in df.columns for col in required_cols):
        st.error(
            "❌ CSV must contain columns: Time (s), X (m/s^2), Y (m/s^2), Z (m/s^2)"
        )
        st.stop()
...
if st.session_state.df is not None:
    ...
    # ==========================
    # ACTIVITY TYPE SELECTION (NEW SECTION)
    # ==========================
    ...
    activity_mode = st.radio(
        ...
        ["Default Activities (walking, toe-rise, running)", "Custom Activities (define your own)"],
        horizontal=True,
        key="activity_mode"
    )
    ...
    # Initialize custom activities in session state
    if 'custom_activities' not in st.session_state:
        st.session_state.custom_activities = [
            {"name": "walking", "start": 0.0, "end": 0.0},
            {"name": "toe_rise", "start": 0.0, "end": 0.0},
            {"name": "running", "start": 0.0, "end": 0.0}
        ]

    # Initialize default preset selection
    if 'default_preset' not in st.session_state:
        st.session_state.default_preset = "Together"

    # ==========================
    # DEFAULT ACTIVITIES MODE
    # ==========================
    if activity_mode == "Default Activities (walking, toe-rise, running)":
        ...
        # Time Range Inputs for Default Activities
        ...
        with col1:
            ...
            walking_end = st.number_input(
                ...
                step=1.0,
                format="%.0f",
                key="walking_end_default"
            )
            # Store in session state for processing
            if len(st.session_state.custom_activities) >= 1:
                st.session_state.custom_activities[0] = {"name": "walking", "start": walking_start, "end": walking_end}
        ...
        with col2:
            ...
            # Store in session state for processing
            if len(st.session_state.custom_activities) >= 2:
                st.session_state.custom_activities[1] = {"name": "toe_rise", "start": toe_start, "end": toe_end}
        ...
        with col3:
            ...
            # Store in session state for processing
            if len(st.session_state.custom_activities) >= 3:
                st.session_state.custom_activities[2] = {"name": "running", "start": run_start, "end": run_end}

        # Ensure we have exactly 3 activities for default mode
        if len(st.session_state.custom_activities) > 3:
            st.session_state.custom_activities = st.session_state.custom_activities[:3]
    ...
    # ==========================
    # CUSTOM ACTIVITIES MODE
    # ==========================
    else:
        st.subheader("🆕 Define Your Custom Activities")
        ...
        with col1:
            num_activities = st.number_input(
                ...
                min_value=2,
                max_value=10,
                value=len(st.session_state.custom_activities),
                step=1,
                key="num_activities"
            )
        ...
        with col2:
            st.write("")
            st.write("")
            if st.button("🔄 Reset Activities", key="reset_custom"):
                st.session_state.custom_activities = [
                    {"name": "activity_1", "start": 0.0, "end": 0.0},
                    {"name": "activity_2", "start": 0.0, "end": 0.0},
                    {"name": "activity_3", "start": 0.0, "end": 0.0}
                ]
                st.rerun()

        # Adjust custom activities list based on number
        if num_activities > len(st.session_state.custom_activities):
            # Add new activities
            for i in range(len(st.session_state.custom_activities), num_activities):
                st.session_state.custom_activities.append(
                    {"name": f"activity_{i + 1}", "start": 0.0, "end": 0.0}
                )
        elif num_activities < len(st.session_state.custom_activities):
            # Remove extra activities
            st.session_state.custom_activities = st.session_state.custom_activities[:num_activities]

        # Display custom activity inputs
        st.markdown("### 📝 Activity Details")

        # Create columns for activities (2 per row)
        num_cols = 2
        for i in range(0, len(st.session_state.custom_activities), num_cols):
            cols = st.columns(num_cols)
            for col_idx in range(num_cols):
                activity_idx = i + col_idx
                if activity_idx < len(st.session_state.custom_activities):
                    with cols[col_idx]:
                        st.markdown(f"#### Activity {activity_idx + 1}")

                        # Activity name
                        activity_name = st.text_input(
                            "Activity name:",
                            value=st.session_state.custom_activities[activity_idx]["name"],
                            key=f"activity_name_{activity_idx}"
                        )
                        ...
                        start_time = st.number_input(
                            "Start time (s):",
                            min_value=0.0,
                            value=float(st.session_state.custom_activities[activity_idx]["start"]),
                            step=1.0,
                            format="%.0f",
                            key=f"activity_start_{activity_idx}"
                        )
                        ...
                        end_time = st.number_input(
                            "End time (s):",
                            min_value=0.0,
                            value=float(st.session_state.custom_activities[activity_idx]["end"]),
                            step=1.0,
                            format="%.0f",
                            key=f"activity_end_{activity_idx}"
                        )

                        # Update session state
                        st.session_state.custom_activities[activity_idx] = {
                            "name": activity_name,
                            "start": start_time,
                            "end": end_time
                        }
        ...
        with col1:
            if st.button("📋 Williamsson Preset", use_container_width=True):
                if len(st.session_state.custom_activities) >= 3:
                    st.session_state.custom_activities[0] = {"name": "walking", "start": 1.0, "end": 99.0}
                    st.session_state.custom_activities[1] = {"name": "toe_rise", "start": 135.0, "end": 166.0}
                    st.session_state.custom_activities[2] = {"name": "running", "start": 210.0, "end": 274.0}
                    st.rerun()

        with col2:
            if st.button("📋 Lama Preset", use_container_width=True):
                if len(st.session_state.custom_activities) >= 3:
                    st.session_state.custom_activities[0] = {"name": "walking", "start": 1.0, "end": 98.0}
                    st.session_state.custom_activities[1] = {"name": "toe_rise", "start": 135.0, "end": 166.0}
                    st.session_state.custom_activities[2] = {"name": "running", "start": 210.0, "end": 270.0}
                    st.rerun()

        with col3:
            if st.button("📋 Together Preset", use_container_width=True):
                if len(st.session_state.custom_activities) >= 3:
                    st.session_state.custom_activities[0] = {"name": "walking", "start": 5.0, "end": 89.0}
                    st.session_state.custom_activities[1] = {"name": "toe_rise", "start": 122.0, "end": 181.0}
                    st.session_state.custom_activities[2] = {"name": "running", "start": 220.0, "end": 274.0}
                    st.rerun()
    ...
    if st.button("🔧 Process Data & Extract Features", type="primary", key="process_button"):
        with st.spinner("Processing data and extracting features..."):
            ...
            activities = st.session_state.custom_activities
            ...
            for activity in activities:
                ...
                if not name:
                    st.warning(f"⚠️ Activity name cannot be empty. Skipping activity.")
                    continue
            ...
            # For backward compatibility in visualization (only for default mode)
            if activity_mode == "Default Activities (walking, toe-rise, running)":
                st.session_state.time_ranges = {
                    "walking_start": valid_activities[0]["start"] if len(valid_activities) > 0 else 0,
                    "walking_end": valid_activities[0]["end"] if len(valid_activities) > 0 else 0,
                    "toe_start": valid_activities[1]["start"] if len(valid_activities) > 1 else 0,
                    "toe_end": valid_activities[1]["end"] if len(valid_activities) > 1 else 0,
                    "run_start": valid_activities[2]["start"] if len(valid_activities) > 2 else 0,
                    "run_end": valid_activities[2]["end"] if len(valid_activities) > 2 else 0
                }
            else:
                # For custom activities, store them in a different format
                st.session_state.time_ranges = {}
                for i, activity in enumerate(valid_activities):
                    st.session_state.time_ranges[f"activity_{i}_start"] = activity["start"]
                    st.session_state.time_ranges[f"activity_{i}_end"] = activity["end"]
                    st.session_state.time_ranges[f"activity_{i}_name"] = activity["name"]

            st.success(f"✅ Extracted {len(features_df)} segments for {len(valid_activities)} activities!")
    ...
    if st.session_state.features_df is not None:
        ...
        with scatter_col1:
            fig_scatter1 = px.scatter(
                ...
                labels={
                    ...
                    "rms_magnitude": "RMS Magnitude (m/s²)",
                    "activity": "Activity"
                },
                height=400,
        ...
        with scatter_col2:
            fig_scatter2 = px.scatter(
                ...
                labels={
                    ...
                    "p2p_magnitude": "Peak-to-Peak (m/s²)",
                    "activity": "Activity"
                },
                height=400,
                hover_data=["segment_start", "segment_end"]
        ...
        with motion_tab4:
            ...
            motion_activity_stats = features_df.groupby("activity")[motion_features].agg(['mean', 'std']).round(3)
            # Add display names
            motion_activity_stats_display = motion_activity_stats.copy()
            motion_activity_stats_display.index = motion_activity_stats_display.index.map(
                lambda x: activity_names.get(x, x.replace('_', ' ').title())
            )
            st.dataframe(motion_activity_stats_display, use_container_width=True)
        ...
        with shape_tab3:
            ...
            st.subheader("Activity-wise Statistics")
            shape_activity_stats = features_df.groupby("activity")[shape_features].agg(['mean', 'std']).round(3)
            # Add display names
            shape_activity_stats_display = shape_activity_stats.copy()
            shape_activity_stats_display.index = shape_activity_stats_display.index.map(
                lambda x: activity_names.get(x, x.replace('_', ' ').title())
            )
            st.dataframe(shape_activity_stats_display, use_container_width=True)
...
<div style='text-align: center; color: gray; font-size: 0.9em;'>
    ...
    <p>🆕 Now with customizable activities!</p>
</div>
